...
<ui:listsTemplate pageTitle="pageTitle.artifactList">
    <h1><fmt:message key="pageTitle.artifactList"/></h1>
    ...
    <table class="dataTable">
        ...
        <tr>
            ...
            <th style="width: 150px;"><fmt:message key="dataTable.heading.compensation"/></th>
            <th style="width: 150px;"><fmt:message key="dataTable.heading.rejectCondition"/></th>
            <th></th>
        </tr>
        </thead>
        <c:forEach items="${artifactList}" var="artifact">
            <tr>
                <td><c:out value="${artifact.artifactId}" /></td>
                <td><c:out value="${artifact.compensation}" /></td>
                <td><c:out value="${artifact.rejectCondition}" /></td>
                <td>
                    <a href="<c:url value='/lists/artifact-definitions/edit.html?id=${artifact.artifactId}&groupid=${selectGroupCommand.researchGroupId}' />"><fmt:message
                            key="link.edit"/></a>
                    <auth:experimenter>
                        <a href="<c:url value='/lists/artifact-definitions/delete.html?id=${artifact.artifactId}&groupid=${selectGroupCommand.researchGroupId}' />"
    ...
    <auth:experimenter>
        <div class="actionBox">
            <a href="<c:url value='add.html?groupid=${selectGroupCommand.researchGroupId}' />" class="lightButtonLink"><fmt:message
                    key="link.addArtifactDefinition"/></a>
        </div>
    </auth:experimenter>

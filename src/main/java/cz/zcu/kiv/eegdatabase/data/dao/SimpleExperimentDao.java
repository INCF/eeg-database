...
public class SimpleExperimentDao<T, PK extends Serializable>
    ...
    // Will be deleted, remained here just for thesis purposes.
    /*
    @Transactional
    @Override
    public List<T> getAllRecordsFull() {
        List<Experiment> experiments = (List<Experiment>) getAllRecords();

        // fetches otherwise lazily loaded collections by multiple calling the initialize() method
        // and passing all DAO getter methods as its parameters
        // SUCCESS for lazy init
        for(Experiment experiment : experiments) {
            Method[] methods = Experiment.class.getDeclaredMethods();
            for (Method method : methods) {
                if(method.getName().startsWith("get") && !method.getReturnType().isPrimitive()) {
                    try {
                        initializeProperty(method.invoke(experiment));
        ...
        // using the Hibernate Template get method to obtain an object from the database
        // FAIL for lazy init
        /*
        List<Experiment> fullExperiments = new ArrayList<Experiment>();
        for(Experiment experiment : experiments) {
            Experiment fullExperiment = (Experiment) getHibernateTemplate().get(type, experiment.getExperimentId());
            fullExperiments.add(fullExperiment);
        }
        */

        // using a single HQL query to get all objects of the specified type.
        // FAIL for lazy init!!!!
        /*
        List<Experiment> eList = getHibernateTemplate().find("from Experiment");
        */

        // Creating custom HQL join queries to fetch desired collections
        // Can work, but its a cumbersome solution
        /*
        String HQLselectArtifact = "SELECT ex FROM Experiment ex LEFT JOIN FETCH ex.artifact artifact LEFT JOIN FETCH ex.scenario scenario";
        String HQLselectScenario = "SELECT ex FROM Experiment ex LEFT JOIN FETCH ex.scenario";
        String HQLselectWeather = "SELECT ex FROM Experiment ex LEFT JOIN FETCH ex.weather weather LEFT JOIN FETCH ex.hardwares hardwares";
        String HQLselectHardwares = "SELECT ex FROM Experiment ex LEFT JOIN FETCH ex.hardwares";
        String HQLselectDatafiles = "SELECT ex FROM Experiment ex LEFT JOIN FETCH ex.datafiles";

        List experimentList = new ArrayList();
        try {
        getSessionFactory().getCurrentSession().createQuery(HQLselectArtifact).list();
        getSessionFactory().getCurrentSession().createQuery(HQLselectScenario).list();
        getSessionFactory().getCurrentSession().createQuery(HQLselectWeather).list();
        getSessionFactory().getCurrentSession().createQuery(HQLselectHardwares).list();
        experimentList = getSessionFactory().getCurrentSession().createQuery(HQLselectDatafiles).list();
        } catch (Exception e) {
            e.printStackTrace();
        }

...
/**
 * Service implementation for Experiment data type. Used in REST service.
 *
 * @author Petr Miko
 */
@Service
public class ExperimentServiceImpl implements ExperimentService {
	...
	public List<HardwareData> getHardwareList() {
		...
		throw new NotImplemented("This cant be implemented as params are loaded from Elasticsearch");
...
//				hardware.getExperiments().add(exp);
	...
	private List<ExperimentData> fillAndSortExperiments(Collection<Experiment> exps) {
		...
		for (Experiment exp : exps) {
			...
			for (GenericParameter param : exp.getGenericParameters("hardware")) {
				HardwareData hw = new HardwareData();
//		                hw.setHardwareId(param.getHardwareId());
//		                hw.setDefaultNumber(param.getDefaultNumber());
				hw.setTitle(param.getValueString());
				for (ParameterAttribute a : param.getAttributes()) {
					if ("type".equals(a.getName())) {
						hw.setType(a.getValue());
					}
				}

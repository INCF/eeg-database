...
INSERT INTO EDUCATION_LEVEL (EDUCATION_LEVEL_ID, "NAME", IS_DEFAULT) VALUES (1, 'NotKnown', 0);
UPDATE PERSON SET EDUCATION_LEVEL_ID=1;
ALTER TABLE PERSON MODIFY EDUCATION_LEVEL_ID NOT NULL;

/* prejmenovani weathernote */
ALTER TABLE EXPERIMENT RENAME COLUMN WEATHERNOTE TO ENVIRONMENT_NOTE;
...
INSERT INTO SUBJECT_GROUP (SUBJECT_GROUP_ID, "NAME", "DESC") VALUES (1, 'Default', 'Default');
...
INSERT INTO ELECTRODE_LOCATION (ELECTRODE_LOCATION_ID, "NAME", SHORTCUT, "DESC", ELECTRODE_TYPE_ID, ELECTRODE_FIX_ID, IS_DEFAULT) VALUES (1, 'None', 'None', 'None', 1, 1, 0);
INSERT INTO ELECTRODE_LOCATION_REL (ELECTRODE_LOCATION_ID, ELECTRODE_CONF_ID) VALUES (1, 1);
INSERT INTO ELECTRODE_TYPE (ELECTRODE_TYPE_ID, "NAME", "DESC",IS_DEFAULT) VALUES (1, 'None', 'None',0);
INSERT INTO ELECTRODE_FIX (ELECTRODE_FIX_ID, "NAME", "DESC",IS_DEFAULT) VALUES (1, 'None', 'None',0);
INSERT INTO ELECTRODE_SYSTEM (ELECTRODE_SYSTEM_ID, "NAME", "DESC", IS_DEFAULT) VALUES (1, 'None', 'None',0);
UPDATE EXPERIMENT SET ELECTRODE_CONF_ID=1;
ALTER TABLE EXPERIMENT MODIFY ELECTRODE_CONF_ID NOT NULL;

/* prevod sampling_rate */
CREATE SEQUENCE DIGITALIZATION_ID_SEQ MINVALUE 1 MAXVALUE 999999999999999999999999999 START WITH 1 INCREMENT BY 1 CACHE 20;
ALTER TABLE DIGITALIZATION MODIFY GAIN FLOAT NULL;
ALTER TABLE DIGITALIZATION MODIFY "FILTER" VARCHAR2(300) NULL;
ALTER TABLE DIGITALIZATION MODIFY SAMPLING_RATE FLOAT NULL;
CREATE OR REPLACE TRIGGER test_trigger BEFORE INSERT ON DIGITALIZATION REFERENCING NEW AS NEW FOR EACH ROW
BEGIN
SELECT DIGITALIZATION_ID_SEQ.nextval INTO :NEW.DIGITALIZATION_ID FROM dual;
END;
/
INSERT INTO DIGITALIZATION(SAMPLING_RATE) VALUES (-1);
INSERT INTO DIGITALIZATION(SAMPLING_RATE) SELECT DISTINCT SAMPLING_RATE FROM DATA_FILE; 
UPDATE DIGITALIZATION SET GAIN=-1, "FILTER"='NotKnown';
DECLARE
...
BEGIN
  ...
  LOOP  
    ...
    if sr_count > 0 then
      ...
      UPDATE EXPERIMENT e SET DIGITALIZATION_ID=(SELECT DIGITALIZATION_ID FROM digitalization dig WHERE dig.SAMPLING_RATE=sampling_rate_type) WHERE e.experiment_id=experiment_row.EXPERIMENT_ID; 
    else
      UPDATE EXPERIMENT e SET DIGITALIZATION_ID=(SELECT digitalization_id from digitalization where sampling_rate=-1) WHERE e.experiment_id=experiment_row.EXPERIMENT_ID;
    end if;
  END LOOP;
END;
/
ALTER TABLE DIGITALIZATION MODIFY GAIN FLOAT NOT NULL;
ALTER TABLE DIGITALIZATION MODIFY "FILTER" VARCHAR2(300) NOT NULL;
ALTER TABLE DIGITALIZATION MODIFY SAMPLING_RATE FLOAT NOT NULL;  
ALTER TABLE EXPERIMENT MODIFY DIGITALIZATION_ID NOT NULL;
DROP SEQUENCE DIGITALIZATION_ID_SEQ;
...
INSERT INTO DISEASE("NAME", "DESC") SELECT DISTINCT DESCRIPTION, DESCRIPTION FROM VISUAL_IMPAIRMENT;
INSERT INTO DISEASE("NAME", "DESC") SELECT DISTINCT DESCRIPTION, DESCRIPTION FROM HEARING_IMPAIRMENT;
...
BEGIN
  ...
  LOOP  
    ...
    LOOP
      ...
      SELECT d.DISEASE_ID into disease_id_type from disease d where d.name=hearing_imp_row.description;
    ...
    loop
      ...
      SELECT d.DISEASE_ID into disease_id_type from disease d where d.name=visual_imp_row.description;
      open experiment_cur (person_row.person_id);
      loop
        fetch experiment_cur into experiment_id_type;
        EXIT WHEN experiment_cur%NOTFOUND;
